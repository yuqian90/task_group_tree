...
})({"5FDo4":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 59162;
...
class TaskInstanceTree extends HTMLElement {
  ...
  renderTree() {
    ...
    function update(source) {
      ...
      // Update the nodes…
      const nodeUpdate = gNode.selectAll("g.task-node").data(treeObj.root.descendants(), d => d.id);
      // Enter any new nodes at the parent's previous position.
      const nodeEnter = nodeUpdate.enter().append("g");
      nodeEnter.attr('class', 'task-node').attr("transform", d => translate(source.y0, source.x0)).attr("fill-opacity", 0).attr("stroke-opacity", 0);
      ...
      const taskRowUpdate = gNode.selectAll("g.node-state-rect-group").data(treeObj.root.descendants(), d => d.id);
      const taskRowEnter = taskRowUpdate.enter().append('g');
      taskRowEnter.merge(taskRowUpdate).attr('class', 'node-state-rect-group');
      ...
      taskRowUpdate.// Update existing rect from the original location
      ...
      const nodeStateRectUpdate = taskRowEnter.merge(taskRowUpdate).selectAll('rect.task-instance-rect,rect.task-group-rect').data(d => d.row, d => d.id);
      const nodeStateRectEnterUpdate = nodeStateRectUpdate.enter().append('rect').merge(nodeStateRectUpdate);
      nodeStateRectEnterUpdate.attr('class', d => d.nodeType == 'TaskGroup' ? 'task-group-rect' : 'task-instance-rect').attr('width', nodeSize).attr('height', nodeSize).on('click', (event, d) => {
        event.preventDefault();
        toggleChecked(d);
        updateSelection();
      ...
      function updateSelection() {
        nodeStateRectEnterUpdate.classed('rect-unchecked', d => !d.checked);
      }
      updateSelection();
      ...
      nodeStateRectUpdate.enter().filter(d => d.node.parent == null).append('text').text(d => d.executionDate.format('YYYYMMDD')).transition().duration(duration).attr('transform', d => `${translate(hScale(d.executionDate) + vSpread / 2, -vSpread / 2)} rotate(-60)`).attr('class', 'axis-label');
    }
  ...
  getExcludedTaskInstances() {
    return Array.from(this.cellStateMap.values()).filter(val => !val.checked && val.nodeType == 'BaseOperator').map(val => {
...
},{}]},["5FDo4","4ee1I"], "4ee1I", "parcelRequire3f0c")
